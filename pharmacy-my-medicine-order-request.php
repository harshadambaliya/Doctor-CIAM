<?php include('include/header.php') ?>

<!-- Dashboard Section Start -->
<section class="ciam-dashboard-section ciam-section">
    <div class="container">
        <div class="row g-0">
            <div class="col col-12 col-md-12 col-lg-4">
                <div class="ciam-dashboard-sidebar">
                    <div class="ciam-dashboard-sidebar-heading">
                        <div class="ciam-sidebar-user">
                            <img src="assets/images/user-img.png" alt="user-img" class="img-fluid">
                            <span>User Name</span>
                        </div>

                        <ul class="cial-sidebar-menu-list">
                            <li class="cial-sidebar-menu-item">
                                <a href="pharmacy-my-medicine-order-request.php" class="cial-sidebar-menu-link active">
                                    <svg id="ballot_black_24dp" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                        <path id="Path_2071" data-name="Path 2071" d="M0,0H24V24H0Z" fill="none"/>
                                        <path id="Path_2072" data-name="Path 2072" d="M13,7.5h5v2H13Zm0,7h5v2H13ZM19,3H5A2.006,2.006,0,0,0,3,5V19a2.006,2.006,0,0,0,2,2H19a2.006,2.006,0,0,0,2-2V5A2.006,2.006,0,0,0,19,3Zm0,16H5V5H19ZM11,6H6v5h5Zm-1,4H7V7h3Zm1,3H6v5h5Zm-1,4H7V14h3Z" fill="#fff"/>
                                    </svg>
                                    <span> My Medicine Order Request</span>
                                </a>
                            </li>
                            <li class="cial-sidebar-menu-item">
                                <a href="pharmacy-my-added-medications.php" class="cial-sidebar-menu-link">
                                    <svg id="category_black_24dp" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                        <path id="Path_2069" data-name="Path 2069" d="M0,0H24V24H0Z" fill="none"/>
                                        <path id="Path_2070" data-name="Path 2070" d="M12,2,6.5,11h11Zm0,3.84L13.93,9H10.06ZM17.5,13A4.5,4.5,0,1,0,22,17.5,4.494,4.494,0,0,0,17.5,13Zm0,7A2.5,2.5,0,1,1,20,17.5,2.5,2.5,0,0,1,17.5,20ZM3,21.5h8v-8H3Zm2-6H9v4H5Z" fill="#fff"/>
                                    </svg>
                                    <span>My Added Medications</span>
                                </a>
                            </li>
                            <li class="cial-sidebar-menu-item">
                                <a href="pharmacy-stock-detail.php" class="cial-sidebar-menu-link">
                                    <svg id="storefront_black_24dp" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                        <g id="Group_1301" data-name="Group 1301">
                                            <rect id="Rectangle_54" data-name="Rectangle 54" width="24" height="24" fill="none"/>
                                        </g>
                                        <g id="Group_1303" data-name="Group 1303">
                                            <g id="Group_1302" data-name="Group 1302">
                                            <path id="Path_2073" data-name="Path 2073" d="M21.9,8.89,20.85,4.52A1.964,1.964,0,0,0,18.94,3H5.05a1.964,1.964,0,0,0-1.9,1.52L2.1,8.89a3.375,3.375,0,0,0,.62,2.88,3.834,3.834,0,0,0,.28.29V19a2.006,2.006,0,0,0,2,2H19a2.006,2.006,0,0,0,2-2V12.06a3.7,3.7,0,0,0,.28-.28A3.364,3.364,0,0,0,21.9,8.89Zm-2.99-3.9,1.05,4.37a1.356,1.356,0,0,1-.25,1.17,1.173,1.173,0,0,1-.94.47,1.25,1.25,0,0,1-1.21-1.14L16.98,5ZM13,5h1.96l.54,4.52a1.344,1.344,0,0,1-.33,1.07,1.193,1.193,0,0,1-.95.41A1.272,1.272,0,0,1,13,9.69ZM8.49,9.52,9.04,5H11V9.69A1.285,1.285,0,0,1,9.71,11a1.2,1.2,0,0,1-.89-.41A1.421,1.421,0,0,1,8.49,9.52ZM4.04,9.36,5.05,5H7.02L6.44,9.86A1.257,1.257,0,0,1,5.23,11a1.151,1.151,0,0,1-.93-.47A1.322,1.322,0,0,1,4.04,9.36ZM5,19V12.97a1.81,1.81,0,0,0,.23.03,3.129,3.129,0,0,0,2.24-.95A3.216,3.216,0,0,0,9.78,13a3.17,3.17,0,0,0,2.23-.93A3.278,3.278,0,0,0,14.3,13a3.175,3.175,0,0,0,2.24-.95,3.129,3.129,0,0,0,2.24.95,1.81,1.81,0,0,0,.23-.03V19Z" fill="#fff"/>
                                            </g>
                                        </g>
                                    </svg>
                                    <span>Stock details</span>
                                </a>
                            </li>
                            <li class="cial-sidebar-menu-item">
                                <a href="pharmacy-stock-upload.php" class="cial-sidebar-menu-link">
                                    <svg id="add_business_black_24dp" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                        <g id="Group_1304" data-name="Group 1304">
                                            <rect id="Rectangle_55" data-name="Rectangle 55" width="24" height="24" fill="none"/>
                                        </g>
                                        <g id="Group_1306" data-name="Group 1306">
                                            <g id="Group_1305" data-name="Group 1305">
                                            <rect id="Rectangle_56" data-name="Rectangle 56" width="15" height="2" transform="translate(2 4)" fill="#fff"/>
                                            <path id="Path_2074" data-name="Path 2074" d="M15,17h2V14h1V12L17,7H2L1,12v2H2v6h9V14h4ZM9,18H4V14H9ZM3.04,12l.6-3H15.36l.6,3Z" fill="#fff"/>
                                            <path id="Path_2075" data-name="Path 2075" d="M23,18H20V15H18v3H15v2h3v3h2V20h3Z" fill="#fff"/>
                                            </g>
                                        </g>
                                    </svg>
                                    <span>Stock Upload</span>
                                </a>
                            </li>
                            <li class="cial-sidebar-menu-item">
                                <a href="pharmacy-my-reviews.php" class="cial-sidebar-menu-link">
                                    <svg id="star_border_black_24dp" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                        <path id="Path_2028" data-name="Path 2028" d="M0,0H24V24H0Z" fill="none"/>
                                        <path id="Path_2029" data-name="Path 2029" d="M22,9.24l-7.19-.62L12,2,9.19,8.63,2,9.24l5.46,4.73L5.82,21,12,17.27,18.18,21l-1.63-7.03ZM12,15.4,8.24,17.67l1-4.28L5.92,10.51l4.38-.38L12,6.1l1.71,4.04,4.38.38L14.77,13.4l1,4.28Z" fill="#fff"/>
                                    </svg>
                                    <span>My Reviews</span>
                                </a>
                            </li>
                            <li class="cial-sidebar-menu-item">
                                <a href="pharmacy-my-wallets.php" class="cial-sidebar-menu-link">
                                    <svg id="account_balance_wallet_black_24dp_2_" data-name="account_balance_wallet_black_24dp (2)" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                        <path id="Path_2076" data-name="Path 2076" d="M0,0H24V24H0Z" fill="none"/>
                                        <path id="Path_2077" data-name="Path 2077" d="M21,7.28V5a2.006,2.006,0,0,0-2-2H5A2,2,0,0,0,3,5V19a2,2,0,0,0,2,2H19a2.006,2.006,0,0,0,2-2V16.72A2,2,0,0,0,22,15V9a2,2,0,0,0-1-1.72ZM20,9v6H13V9ZM5,19V5H19V7H13a2.006,2.006,0,0,0-2,2v6a2.006,2.006,0,0,0,2,2h6v2Z" fill="#fff"/>
                                        <circle id="Ellipse_651" data-name="Ellipse 651" cx="1.5" cy="1.5" r="1.5" transform="translate(14.5 10.5)" fill="#fff"/>
                                    </svg>
                                    <span>My Wallet</span>
                                </a>
                            </li>
                            <li class="cial-sidebar-menu-item">
                                <a href="pharmacy-ledger.php" class="cial-sidebar-menu-link">
                                    <svg id="receipt_long_black_24dp" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                        <path id="Path_2055" data-name="Path 2055" d="M0,0H24V24H0Z" fill="none"/>
                                        <g id="Group_1083" data-name="Group 1083">
                                            <path id="Path_2056" data-name="Path 2056" d="M19.5,3.5,18,2,16.5,3.5,15,2,13.5,3.5,12,2,10.5,3.5,9,2,7.5,3.5,6,2V16H3v3a3,3,0,0,0,3,3H18a3,3,0,0,0,3-3V2ZM15,20H6a1,1,0,0,1-1-1V18H15Zm4-1a1,1,0,0,1-2,0V16H8V5H19Z" fill="#fff"/>
                                            <rect id="Rectangle_42" data-name="Rectangle 42" width="6" height="2" transform="translate(9 7)" fill="#fff"/>
                                            <rect id="Rectangle_43" data-name="Rectangle 43" width="2" height="2" transform="translate(16 7)" fill="#fff"/>
                                            <rect id="Rectangle_44" data-name="Rectangle 44" width="6" height="2" transform="translate(9 10)" fill="#fff"/>
                                            <rect id="Rectangle_45" data-name="Rectangle 45" width="2" height="2" transform="translate(16 10)" fill="#fff"/>
                                        </g>
                                    </svg>
                                    <span>Ledger</span>
                                </a>
                            </li>
                            <li class="cial-sidebar-menu-item">
                                <a href="pharmacy-account-setting.php" class="cial-sidebar-menu-link">
                                    <svg id="manage_accounts_black_24dp" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                        <g id="Group_1071" data-name="Group 1071">
                                            <path id="Path_2041" data-name="Path 2041" d="M0,0H24V24H0Z" fill="none"/>
                                        </g>
                                        <g id="Group_1073" data-name="Group 1073">
                                            <g id="Group_1072" data-name="Group 1072">
                                            <path id="Path_2042" data-name="Path 2042" d="M4,18v-.65a.946.946,0,0,1,.41-.81A10.889,10.889,0,0,1,10,15a.22.22,0,0,1,.08.01,6.886,6.886,0,0,1,.59-1.98c-.22-.02-.44-.03-.67-.03a12.876,12.876,0,0,0-6.61,1.82A2.922,2.922,0,0,0,2,17.35V20h9.26a7.133,7.133,0,0,1-.97-2Z" fill="#fff"/>
                                            <path id="Path_2043" data-name="Path 2043" d="M10,12A4,4,0,1,0,6,8,4,4,0,0,0,10,12Zm0-6A2,2,0,1,1,8,8,2.006,2.006,0,0,1,10,6Z" fill="#fff"/>
                                            <path id="Path_2044" data-name="Path 2044" d="M20.75,16a4.338,4.338,0,0,0-.06-.63l1.14-1.01-1-1.73-1.45.49a3.647,3.647,0,0,0-1.08-.63L18,11H16l-.3,1.49a3.647,3.647,0,0,0-1.08.63l-1.45-.49-1,1.73,1.14,1.01a4.338,4.338,0,0,0-.06.63,4.338,4.338,0,0,0,.06.63l-1.14,1.01,1,1.73,1.45-.49a3.647,3.647,0,0,0,1.08.63L16,21h2l.3-1.49a3.647,3.647,0,0,0,1.08-.63l1.45.49,1-1.73-1.14-1.01A4.338,4.338,0,0,0,20.75,16ZM17,18a2,2,0,1,1,2-2A2.006,2.006,0,0,1,17,18Z" fill="#fff"/>
                                            </g>
                                        </g>
                                    </svg>
                                    <span>Account Setting</span>
                                </a>
                            </li>
                            <li class="cial-sidebar-menu-item">
                                <a href="#" class="cial-sidebar-menu-link">
                                    <svg id="logout_black_24dp" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                        <g id="Group_990" data-name="Group 990">
                                            <path id="Path_2021" data-name="Path 2021" d="M0,0H24V24H0Z" fill="none"/>
                                        </g>
                                        <g id="Group_991" data-name="Group 991">
                                            <path id="Path_2022" data-name="Path 2022" d="M17,8,15.59,9.41,17.17,11H9v2h8.17l-1.58,1.58L17,16l4-4ZM5,5h7V3H5A2.006,2.006,0,0,0,3,5V19a2.006,2.006,0,0,0,2,2h7V19H5Z" fill="#fff"/>
                                        </g>
                                    </svg>
                                    <span>Log Out</span>
                                </a>
                            </li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="col col-12 col-md-12 col-lg-8">
                <div class="ciam-dashboard-content">
                    <div class="ciam-dashboard-content__inner">
                        <div class="ciam-medicine-order-card">
                            <div class="row">
                                <div class="col col-12 col-md-4 col-lg-4">
                                    <div class="ciam-medicine-order-img">
                                        <img src="assets/images/medisince-img.png" alt="" class="img-fluid">
                                    </div>
                                </div>
                                <div class="col col-12 col-md-8 col-lg-8">
                                    <div class="ciam-medicine-order-card-body">
                                        <div class="ciam-medicine-patient-detail">
                                            <img src="assets/images/patient-profile-img.png" alt="patient-profile-img" class="img-fluid">
                                            <h3 class="ciam-medicine-patient-name">Patient Name</h3>
                                        </div>
                                        <div class="row">
                                            <div class="col col-12 col-md-12 col-lg-6">
                                                <div class="ciam-medicine-order-detail">
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Booking Id :</span>
                                                        <span class="ciam-card-details-text">9956328</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">No. of Medications :</span>
                                                        <span class="ciam-card-details-text">09</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Order Prepared :</span>
                                                        <span class="ciam-card-details-text">packed</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Amount of order :</span>
                                                        <span class="ciam-card-details-text">$149</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col col-12 col-md-12 col-lg-6">
                                                <div class="ciam-medicine-order-detail">
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Medicine Name :</span>
                                                        <span class="ciam-card-details-text">date</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Intake Directions :</span>
                                                        <span class="ciam-card-details-text">2 Days</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Delivery Location :</span>
                                                        <span class="ciam-card-details-text">location</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Invoice :</span>
                                                        <div class="ciam-card-details-text">
                                                            <div class="ciam-fileInput">
                                                                <input type="file" class="form-control">
                                                                <span class="ciam-fileInput-text">
                                                                    <svg id="file_upload_black_24dp_1_" data-name="file_upload_black_24dp (1)" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                                                        <g id="Group_1332" data-name="Group 1332">
                                                                            <rect id="Rectangle_57" data-name="Rectangle 57" width="24" height="24" fill="none"/>
                                                                        </g>
                                                                        <g id="Group_1333" data-name="Group 1333">
                                                                            <path id="Path_2078" data-name="Path 2078" d="M18,15v3H6V15H4v3a2.006,2.006,0,0,0,2,2H18a2.006,2.006,0,0,0,2-2V15ZM7,9l1.41,1.41L11,7.83V16h2V7.83l2.59,2.58L17,9,12,4Z" fill="#409690"/>
                                                                        </g>
                                                                    </svg>
                                                                </span>
                                                            </div>
                                                        </div>
                                                        <a href="assets/images/dummy.pdf" download>
                                                            <svg id="file_download_black_24dp_2_" data-name="file_download_black_24dp (2)" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                                                <g id="Group_1246" data-name="Group 1246">
                                                                    <rect id="Rectangle_53" data-name="Rectangle 53" width="24" height="24" fill="none"/>
                                                                </g>
                                                                <g id="Group_1247" data-name="Group 1247">
                                                                    <path id="Path_2068" data-name="Path 2068" d="M18,15v3H6V15H4v3a2.006,2.006,0,0,0,2,2H18a2.006,2.006,0,0,0,2-2V15Zm-1-4L15.59,9.59,13,12.17V4H11v8.17L8.41,9.59,7,11l5,5Z" fill="#409690"/>
                                                                </g>
                                                            </svg>
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="ciam-delivery-agent-detail">
                                            <h4 class="ciam-delivery-agent-title">Delivery Agent Details :</h4>
                                            <div class="ciam-delivery-agent-content">
                                                <div class="ciam-delivery-agent-img">
                                                    <img src="assets/images/delivery-agent-img.png" alt="delivery-agent-img" class="img-fluid">
                                                </div>
                                                <div class="ciam-delivery-agent-content__inner">
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Delivery agent Name :</span>
                                                        <span class="ciam-card-details-text">Name</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Delivery agent contact details :</span>
                                                        <a href="tel:+919876543210" class="ciam-card-details-text">+919876543210</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="ciam-medicine-order-card">
                            <div class="row">
                                <div class="col col-12 col-md-4 col-lg-4">
                                    <div class="ciam-medicine-order-img">
                                        <img src="assets/images/medisince-img.png" alt="" class="img-fluid">
                                    </div>
                                </div>
                                <div class="col col-12 col-md-8 col-lg-8">
                                    <div class="ciam-medicine-order-card-body">
                                        <div class="ciam-medicine-patient-detail">
                                            <img src="assets/images/patient-profile-img.png" alt="patient-profile-img" class="img-fluid">
                                            <h3 class="ciam-medicine-patient-name">Patient Name</h3>
                                        </div>
                                        <div class="row">
                                            <div class="col col-12 col-md-12 col-lg-6">
                                                <div class="ciam-medicine-order-detail">
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Booking Id :</span>
                                                        <span class="ciam-card-details-text">9956328</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">No. of Medications :</span>
                                                        <span class="ciam-card-details-text">09</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Order Prepared :</span>
                                                        <span class="ciam-card-details-text">packed</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Amount of order :</span>
                                                        <span class="ciam-card-details-text">$149</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col col-12 col-md-12 col-lg-6">
                                                <div class="ciam-medicine-order-detail">
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Medicine Name :</span>
                                                        <span class="ciam-card-details-text">date</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Intake Directions :</span>
                                                        <span class="ciam-card-details-text">2 Days</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Delivery Location :</span>
                                                        <span class="ciam-card-details-text">location</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Invoice :</span>
                                                        <div class="ciam-card-details-text">
                                                            <div class="ciam-fileInput">
                                                                <input type="file" class="form-control">
                                                                <span class="ciam-fileInput-text">
                                                                    <svg id="file_upload_black_24dp_1_" data-name="file_upload_black_24dp (1)" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                                                        <g id="Group_1332" data-name="Group 1332">
                                                                            <rect id="Rectangle_57" data-name="Rectangle 57" width="24" height="24" fill="none"/>
                                                                        </g>
                                                                        <g id="Group_1333" data-name="Group 1333">
                                                                            <path id="Path_2078" data-name="Path 2078" d="M18,15v3H6V15H4v3a2.006,2.006,0,0,0,2,2H18a2.006,2.006,0,0,0,2-2V15ZM7,9l1.41,1.41L11,7.83V16h2V7.83l2.59,2.58L17,9,12,4Z" fill="#409690"/>
                                                                        </g>
                                                                    </svg>
                                                                </span>
                                                            </div>
                                                        </div>
                                                        <a href="assets/images/dummy.pdf" download>
                                                            <svg id="file_download_black_24dp_2_" data-name="file_download_black_24dp (2)" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                                                <g id="Group_1246" data-name="Group 1246">
                                                                    <rect id="Rectangle_53" data-name="Rectangle 53" width="24" height="24" fill="none"/>
                                                                </g>
                                                                <g id="Group_1247" data-name="Group 1247">
                                                                    <path id="Path_2068" data-name="Path 2068" d="M18,15v3H6V15H4v3a2.006,2.006,0,0,0,2,2H18a2.006,2.006,0,0,0,2-2V15Zm-1-4L15.59,9.59,13,12.17V4H11v8.17L8.41,9.59,7,11l5,5Z" fill="#409690"/>
                                                                </g>
                                                            </svg>
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="ciam-delivery-agent-detail">
                                            <h4 class="ciam-delivery-agent-title">Delivery Agent Details :</h4>
                                            <div class="ciam-delivery-agent-content">
                                                <div class="ciam-delivery-agent-img">
                                                    <img src="assets/images/delivery-agent-img.png" alt="delivery-agent-img" class="img-fluid">
                                                </div>
                                                <div class="ciam-delivery-agent-content__inner">
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Delivery agent Name :</span>
                                                        <span class="ciam-card-details-text">Name</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Delivery agent contact details :</span>
                                                        <a href="tel:+919876543210" class="ciam-card-details-text">+919876543210</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="ciam-medicine-order-card">
                            <div class="row">
                                <div class="col col-12 col-md-4 col-lg-4">
                                    <div class="ciam-medicine-order-img">
                                        <img src="assets/images/medisince-img.png" alt="" class="img-fluid">
                                    </div>
                                </div>
                                <div class="col col-12 col-md-8 col-lg-8">
                                    <div class="ciam-medicine-order-card-body">
                                        <div class="ciam-medicine-patient-detail">
                                            <img src="assets/images/patient-profile-img.png" alt="patient-profile-img" class="img-fluid">
                                            <h3 class="ciam-medicine-patient-name">Patient Name</h3>
                                        </div>
                                        <div class="row">
                                            <div class="col col-12 col-md-12 col-lg-6">
                                                <div class="ciam-medicine-order-detail">
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Booking Id :</span>
                                                        <span class="ciam-card-details-text">9956328</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">No. of Medications :</span>
                                                        <span class="ciam-card-details-text">09</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Order Prepared :</span>
                                                        <span class="ciam-card-details-text">packed</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Amount of order :</span>
                                                        <span class="ciam-card-details-text">$149</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col col-12 col-md-12 col-lg-6">
                                                <div class="ciam-medicine-order-detail">
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Medicine Name :</span>
                                                        <span class="ciam-card-details-text">date</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Intake Directions :</span>
                                                        <span class="ciam-card-details-text">2 Days</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Delivery Location :</span>
                                                        <span class="ciam-card-details-text">location</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Invoice :</span>
                                                        <div class="ciam-card-details-text">
                                                            <div class="ciam-fileInput">
                                                                <input type="file" class="form-control">
                                                                <span class="ciam-fileInput-text">
                                                                    <svg id="file_upload_black_24dp_1_" data-name="file_upload_black_24dp (1)" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                                                        <g id="Group_1332" data-name="Group 1332">
                                                                            <rect id="Rectangle_57" data-name="Rectangle 57" width="24" height="24" fill="none"/>
                                                                        </g>
                                                                        <g id="Group_1333" data-name="Group 1333">
                                                                            <path id="Path_2078" data-name="Path 2078" d="M18,15v3H6V15H4v3a2.006,2.006,0,0,0,2,2H18a2.006,2.006,0,0,0,2-2V15ZM7,9l1.41,1.41L11,7.83V16h2V7.83l2.59,2.58L17,9,12,4Z" fill="#409690"/>
                                                                        </g>
                                                                    </svg>
                                                                </span>
                                                            </div>
                                                        </div>
                                                        <a href="assets/images/dummy.pdf" download>
                                                            <svg id="file_download_black_24dp_2_" data-name="file_download_black_24dp (2)" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                                                <g id="Group_1246" data-name="Group 1246">
                                                                    <rect id="Rectangle_53" data-name="Rectangle 53" width="24" height="24" fill="none"/>
                                                                </g>
                                                                <g id="Group_1247" data-name="Group 1247">
                                                                    <path id="Path_2068" data-name="Path 2068" d="M18,15v3H6V15H4v3a2.006,2.006,0,0,0,2,2H18a2.006,2.006,0,0,0,2-2V15Zm-1-4L15.59,9.59,13,12.17V4H11v8.17L8.41,9.59,7,11l5,5Z" fill="#409690"/>
                                                                </g>
                                                            </svg>
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="ciam-delivery-agent-detail">
                                            <h4 class="ciam-delivery-agent-title">Delivery Agent Details :</h4>
                                            <div class="ciam-delivery-agent-content">
                                                <div class="ciam-delivery-agent-img">
                                                    <img src="assets/images/delivery-agent-img.png" alt="delivery-agent-img" class="img-fluid">
                                                </div>
                                                <div class="ciam-delivery-agent-content__inner">
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Delivery agent Name :</span>
                                                        <span class="ciam-card-details-text">Name</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Delivery agent contact details :</span>
                                                        <a href="tel:+919876543210" class="ciam-card-details-text">+919876543210</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="ciam-medicine-order-card">
                            <div class="row">
                                <div class="col col-12 col-md-4 col-lg-4">
                                    <div class="ciam-medicine-order-img">
                                        <img src="assets/images/medisince-img.png" alt="" class="img-fluid">
                                    </div>
                                </div>
                                <div class="col col-12 col-md-8 col-lg-8">
                                    <div class="ciam-medicine-order-card-body">
                                        <div class="ciam-medicine-patient-detail">
                                            <img src="assets/images/patient-profile-img.png" alt="patient-profile-img" class="img-fluid">
                                            <h3 class="ciam-medicine-patient-name">Patient Name</h3>
                                        </div>
                                        <div class="row">
                                            <div class="col col-12 col-md-12 col-lg-6">
                                                <div class="ciam-medicine-order-detail">
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Booking Id :</span>
                                                        <span class="ciam-card-details-text">9956328</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">No. of Medications :</span>
                                                        <span class="ciam-card-details-text">09</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Order Prepared :</span>
                                                        <span class="ciam-card-details-text">packed</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Amount of order :</span>
                                                        <span class="ciam-card-details-text">$149</span>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col col-12 col-md-12 col-lg-6">
                                                <div class="ciam-medicine-order-detail">
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Medicine Name :</span>
                                                        <span class="ciam-card-details-text">date</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Intake Directions :</span>
                                                        <span class="ciam-card-details-text">2 Days</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Delivery Location :</span>
                                                        <span class="ciam-card-details-text">location</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Invoice :</span>
                                                        <div class="ciam-card-details-text">
                                                            <div class="ciam-fileInput">
                                                                <input type="file" class="form-control">
                                                                <span class="ciam-fileInput-text">
                                                                    <svg id="file_upload_black_24dp_1_" data-name="file_upload_black_24dp (1)" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                                                        <g id="Group_1332" data-name="Group 1332">
                                                                            <rect id="Rectangle_57" data-name="Rectangle 57" width="24" height="24" fill="none"/>
                                                                        </g>
                                                                        <g id="Group_1333" data-name="Group 1333">
                                                                            <path id="Path_2078" data-name="Path 2078" d="M18,15v3H6V15H4v3a2.006,2.006,0,0,0,2,2H18a2.006,2.006,0,0,0,2-2V15ZM7,9l1.41,1.41L11,7.83V16h2V7.83l2.59,2.58L17,9,12,4Z" fill="#409690"/>
                                                                        </g>
                                                                    </svg>
                                                                </span>
                                                            </div>
                                                        </div>
                                                        <a href="assets/images/dummy.pdf" download>
                                                            <svg id="file_download_black_24dp_2_" data-name="file_download_black_24dp (2)" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
                                                                <g id="Group_1246" data-name="Group 1246">
                                                                    <rect id="Rectangle_53" data-name="Rectangle 53" width="24" height="24" fill="none"/>
                                                                </g>
                                                                <g id="Group_1247" data-name="Group 1247">
                                                                    <path id="Path_2068" data-name="Path 2068" d="M18,15v3H6V15H4v3a2.006,2.006,0,0,0,2,2H18a2.006,2.006,0,0,0,2-2V15Zm-1-4L15.59,9.59,13,12.17V4H11v8.17L8.41,9.59,7,11l5,5Z" fill="#409690"/>
                                                                </g>
                                                            </svg>
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="ciam-delivery-agent-detail">
                                            <h4 class="ciam-delivery-agent-title">Delivery Agent Details :</h4>
                                            <div class="ciam-delivery-agent-content">
                                                <div class="ciam-delivery-agent-img">
                                                    <img src="assets/images/delivery-agent-img.png" alt="delivery-agent-img" class="img-fluid">
                                                </div>
                                                <div class="ciam-delivery-agent-content__inner">
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Delivery agent Name :</span>
                                                        <span class="ciam-card-details-text">Name</span>
                                                    </div>
                                                    <div class="ciam-card-details">
                                                        <span class="ciam-card-details-title">Delivery agent contact details :</span>
                                                        <a href="tel:+919876543210" class="ciam-card-details-text">+919876543210</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- Dashboard Section End -->

<?php include('include/footer.php') ?>